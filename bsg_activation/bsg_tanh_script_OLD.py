...
def bsg_exponential_main_initial(angbitlen, ansbitlen, negprec, posprec, extriter, precision):
    ...
    module bsg_tanh #(parameter neg_prec_p=%(n)d, posi_prec_p=%(p)d, extr_iter_p=1, ans_width_p = %(s)d, ang_width_p = %(g)d, precision=%(c)d)
    (
    input clk_i
    ...
    """ %{'n': negprec, 'p': posprec, 's':ansbitlen, 'g':angbitlen, 'c':precision})
    return
